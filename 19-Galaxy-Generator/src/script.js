...
parameters.spin = 1;
parameters.randomness = 1.31;
parameters.randomnessPower = 3.33;
parameters.insideColor = '#ff6030';
parameters.outsideColor = '#1b3894';
...
const generateGalaxy = () => {
    ...
    for( let i = 0 ; i < parameters.count ; i++){
        ...
        // Positioning random values in array
        const i3 = i*3;

        const radius = Math.random() * parameters.radius;
        const angle =  (i % parameters.branches) * Math.PI * 2 / parameters.branches;
...
gui.add(parameters,'size').min(0.01).max(0.1).step(.0001).onFinishChange(generateGalaxy);
gui.add(parameters,'radius').min(0.001).max(20).step(.01).onFinishChange(generateGalaxy);
...
gui.add(parameters,'randomnessPower').min(1).max(5).step(.01).onFinishChange(generateGalaxy);
